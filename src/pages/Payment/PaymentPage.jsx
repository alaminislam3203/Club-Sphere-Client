...
const PaymentPage = () => {
  ...
  // merged state
  const { price, planName, checkoutUrl } = location.state || {};
  ...
  return (
    <section className="bg-slate-50 py-16 min-h-screen flex items-center font-sans">
      ...
      >
        ...
        {/* Header */}
        <div className="text-center mb-10 relative z-10">
          <div className="w-20 h-20 bg-blue-50 text-[#0b99ce] rounded-[2rem] flex items-center justify-center mx-auto mb-6 text-3xl shadow-inner">
            {finalCost === 0 ? (
              <FaCheckCircle className="text-emerald-500" />
            ) : (
              <FaCreditCard />
        ...
        {/* Summary */}
        <div className="bg-slate-50 p-8 rounded-[2rem] space-y-5 mb-10 border border-slate-100">
          <div className="flex justify-between items-center">
            <span className="flex items-center gap-2 uppercase text-[10px] font-black tracking-[0.15em] text-slate-400">
              <FaTag className="text-[#0b99ce]" />
              {t('plan_selected', 'Plan Selected')}
            ...
            <span className="text-slate-800 font-black bg-white px-4 py-1 rounded-lg shadow-sm">
              {planName}
            </span>
          </div>
        ...
        <button
          onClick={handlePaymentInitiation}
          disabled={isProcessing}
          className={`w-full py-6 rounded-2xl font-black text-lg uppercase tracking-widest text-white shadow-2xl transition-all duration-500 flex items-center justify-center gap-4
          ${
            isProcessing
              ? 'bg-slate-300 cursor-not-allowed'
              : finalCost === 0
                ? 'bg-emerald-500 hover:bg-emerald-600'
                : 'bg-[#0b99ce] hover:bg-[#fe3885]'
          }`}
        >
          {isProcessing ? (
            <>
